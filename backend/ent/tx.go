...
func (tx *Tx) init() {
	...
	tx.Instances = NewInstancesClient(tx.config)
	tx.LoadBalancer = NewLoadBalancerClient(tx.config)
	tx.ObjectStorage = NewObjectStorageClient(tx.config)
	tx.Panier = NewPanierClient(tx.config)
	tx.PanierBlockStorage = NewPanierBlockStorageClient(tx.config)
	tx.PanierIPAddress = NewPanierIPAddressClient(tx.config)
	tx.PanierInstances = NewPanierInstancesClient(tx.config)
	tx.PanierLoadBalancer = NewPanierLoadBalancerClient(tx.config)
	tx.PanierObjectStorage = NewPanierObjectStorageClient(tx.config)
}
