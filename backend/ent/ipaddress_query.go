...
func (iaq *IPAddressQuery) QueryPanierIPAddress() *PanierIPAddressQuery {
	query := (&PanierIPAddressClient{config: iaq.config}).Query()
...
func (iaq *IPAddressQuery) Clone() *IPAddressQuery {
	...
	return &IPAddressQuery{
		...
		withPanierIPAddress: iaq.withPanierIPAddress.Clone(),
...
func (iaq *IPAddressQuery) WithPanierIPAddress(opts ...func(*PanierIPAddressQuery)) *IPAddressQuery {
	query := (&PanierIPAddressClient{config: iaq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	iaq.withPanierIPAddress = query
	return iaq
}
...
func (iaq *IPAddressQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*IPAddress, error) {
	...
	if query := iaq.withPanierIPAddress; query != nil {
		if err := iaq.loadPanierIPAddress(ctx, query, nodes,
			func(n *IPAddress) { n.Edges.PanierIPAddress = []*PanierIPAddress{} },
			func(n *IPAddress, e *PanierIPAddress) { n.Edges.PanierIPAddress = append(n.Edges.PanierIPAddress, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (iaq *IPAddressQuery) loadPanierIPAddress(ctx context.Context, query *PanierIPAddressQuery, nodes []*IPAddress, init func(*IPAddress), assign func(*IPAddress, *PanierIPAddress)) error {
	...
	query.Where(predicate.PanierIPAddress(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(ipaddress.PanierIPAddressColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n._IPAddress_id
		if fk == nil {
			return fmt.Errorf(`foreign-key "_IPAddress_id" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "_IPAddress_id" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
